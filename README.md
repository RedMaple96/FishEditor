...
- **多路径导入与游动模拟**：支持批量上传已生成的 `.dat` 文件，可在画布中以半透明线条区分并同时显示多条路径，且支持为每条导入的路径独立控制游动播放与速度（1~100 级实时调节），进行多条鱼同时游动的群游效果模拟测试。
...
7. **多路径导入与模拟测试**：
   - 在左侧面板点击展开 **导入与多路径模拟**。
   - 点击上传按钮，可单选或**多选**（批量）上传本地的 `.dat` 路径文件。
   - 导入后每条路径都会分配一种区别颜色并显示在列表中，您可以**独立调节**任意一条导入路径的“游动速度”，独立控制该路径上小鱼的“播放/停止”。
   - 可以同时点击多条路径的“开始模拟”，在右侧画布观察它们群游的效果。
8. **导出数据**：展开左侧面板的 **导出路径数据**，输入文件名，点击相应的导出按钮下载 `.dat`（仅数据）或 `.png`（带截图）。
